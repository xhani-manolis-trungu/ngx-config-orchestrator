...
export class ConfigurationService {
  ...
  private readonly appSettingsToken = inject(APP_SETTINGS);
  private readonly authSettingsToken = inject(IAUTH_SETTINGS, { optional: true });
  ...
  public loadSettings(): Observable<boolean> {
    return this.loader.loadManifest().pipe(
      map((remoteSettings) => {
        const mergedSettings = deepMerge({ ...this.environment }, remoteSettings);
      ...
      catchError((error) => {
        console.error('ngx-config-orchestrator: Failed to load configuration.', error);
        return of(false);
      })
    );
  }
