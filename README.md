...
- **Dependency Orchestration**: Initialize other services (like Logging or Analytics) only *after* configuration is loaded.
...
@Component({
  template: `
    ...
    <p>API: {{ config().api_url }}</p>
  `
})
export class AppComponent {
  // Expose signal to template
  public config = inject(ConfigurationService).settings;
}
...
{
  "auth_settings": {
    "authority": "[https://identity.myapp.com](https://identity.myapp.com)",
...
import { IAUTH_SETTINGS, IAuthSettings } from 'ngx-config-orchestrator';

@Injectable()
export class AuthService {
  constructor(@Inject(IAUTH_SETTINGS) private authConfig: IAuthSettings) {
    console.log('Authority URL:', authConfig.authority);
  }
}
...
```typescript
{
  "api_url": "[https://api.staging.com](https://api.staging.com)"
}
...
function initAnalytics(configService: ConfigurationService, analytics: AnalyticsService) {
  return () => {
      ...
      dependencies: [
        {
          ...
          deps: [ConfigurationService, AnalyticsService],
          multi: true
        }
      ]
    })
...
The `test-app` is a demo application included in the workspace to verify functionality.

```bash
# Serve the test application
npm run ng -- serve test-app
...
- **Hot Test:** Modify `projects/test-app/src/assets/app-settings.manifest.json` while the server is running and refresh the browser to see changes instantly.
